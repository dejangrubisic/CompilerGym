...
import compiler_gym.envs.loop_tool.paths
import compiler_gym.third_party.llvm as llvm

# from compiler_gym.util.commands import Popen, run_command
from compiler_gym.envs.loop_tool.service.utils import (
    print_list,
    proto_buff_container_to_list,
    run_command,
    run_command_get_stderr,
    run_command_stdout_redirect,
)
...
class Environment:
    def __init__(
        self, working_directory: Path, benchmark: Benchmark, timeout_sec: float
    ):
    ...
    def apply_action(self, action: str, save_state: bool) -> bool:
        ...
        if action == "up":
            ...
            tree_after = self.update_tree(
                tree_before.try_swap(self.cursor, tree_before.previous_ref(self.cursor))
            )

        elif action == "swap_down":
            tree_after = self.update_tree(
                tree_before.try_swap(self.cursor, tree_before.next_ref(self.cursor))
            )
            ...
            try:
                tree_after = self.update_tree(
                    tree_before.split(self.cursor, split_param)
                )
            except:
                pass
            ...
            tree_after = self.update_tree(
                tree_before.annotate(self.cursor, "vectorize")
            )
